...
export const api = reactive({
  ...
  async uploadFile(file, playlist, duration) {
    ...
    try {
      ...
      this.deployPlaylist(playlist, file.name);
    } catch (error) {
      console.error(error);
    }
  },
  ...
  async getDetailedPlaylists(name) {
    try {
      ...
      const url = name
        ? `/playlists/${name}?token=${apiToken}`
        : `/playlists?token=${apiToken}`;
  ...
  async addToPlaylist(playlistName, assetsName, duration) {
    ...
    try {
      const apiToken = localStorage.getItem("apiToken");
      const playlistInfo = await apiBuilder.get(
        `/playlists/${playlistName}?token=${apiToken}`
      );
      ...
      const response = await axios.post(
        `https://lvll3gliic.pisignage.com/api/playlists/${playlistName}?token=${apiToken}`,
        data,
        { headers }
      );
      console.log(response);
  ...
  async getUpdatedHtmlFile(
    ...
    try {
      axios
        .get(
          `http://localhost:3001/api/get-html-file/${templateName}&${heading}&${text}`
        )
        .then((responseHtml) => {
          console.log("RESPONSE: " + responseHtml.data.html);
          const blob = new Blob([responseHtml.data.html], {
            type: "text/html",
          });
          const htmlFile = new File([blob], `${file_name}.html`, {
            type: "text/html",
          });
  ...
  async controlTv(status, id) {
    try {
      ...
      const test = await axios.post(
        `https://lvll3gliic.pisignage.com/api/pitv/${id}?token=${apiToken}`,
        headers
      );
      console.log(test);
    } catch (error) {
